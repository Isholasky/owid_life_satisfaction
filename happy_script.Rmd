...
## Import data
cantril <- 
  read_csv(here("data/subjective-happiness.csv")) %>%
  clean_names() %>% 
  rename(country = entity) %>% 
  rename(happiness = happiness_happy_aggregate)
```
...
```{r}
cantril %>% 
  mutate(`Happiness score` = round(happiness, 2)) %>%
  plot_geo(width = 940) %>% 
  add_trace(
    z = ~ happiness,
    color = ~ happiness,
...
cantril_chart <- 
  ...
  scale_x_continuous(limits = c(1993, 2022),
                     breaks = c(1993,1995,2000,2005,2010,2015,2020,2022)) +
